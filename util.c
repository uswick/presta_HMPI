...
/* This barrier implementation is taken from the Sphinx MPI benchmarks
   and is based directly on the MPICH basddarrier implementation...        */
...
void generic_barrier (MPI_Comm curcomm)
{
  ...
  if ( size > 1 ) {
    ...
    //printf("Generic barrier [enter barrier]  rank: %d  size: %d  \n",rank,size);
    ...
    if ( rank < N2_prev ) {
      if( rank < surfeit ) {
        ...
        dst = N2_prev + rank;
        //printf("Generic barrier [MPI_recv ]  rank: %d N2prev :%d surefit: %d   dest: %d  \n",rank,N2_prev,surfeit,dst);
      ...
      for (d = 1; d < N2_prev; d <<= 1) {
        ...
        //printf("Generic barrier [MPI_SendRecv() ]  rank: %d N2prev :%d  dest: %d  \n",rank,N2_prev,dst);
      ...
      if ( rank < surfeit ) {
        dst = N2_prev + rank;
        //printf("Generic barrier [MPI_Send ]  rank: %d N2prev :%d surefit: %d   dest: %d  \n",rank,N2_prev,surfeit,dst);
