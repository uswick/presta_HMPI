...
int main(int argc, char *argv[])
{
  ...
  printf("Broadcasting of shared mem region complete! rank : %d  region_addr : %p region_ptr : %p \n", rank, &req_array, &req_array);
                     ...
                     int useBarrier, int useNearestRank, MPI_Comm *activeComm)
{
                    ...
                    allocPattern, useNearestRank) )
  {
    if(bufsize < BLOCKSIZE){
        posix_memalign((void**)&comBuf, LINESIZE, bufsize + sizeof(int));

        /*if(!IS_SM_BUF((void*)comBuf)){
                    printf("Unicom Test : Failed to allocate from SM region :%p \n", comBuf);
                    exit(-1);
        }*/
    ...
    else 
    {
      ...
      for ( i = 0; i < iters; i++ )
      {
            if(bufsize < BLOCKSIZE){
              ...
              //reset the flag for sender for the purpose of next iteration
              req_array[currtarg].sync[0] = 1;
